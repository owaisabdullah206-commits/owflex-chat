...
export async function sendCreditGraceEmail({
  ...
  try {
    await brevo.transactionalEmails.sendTransacEmail({
      sender: BREVO_SENDER,
      to: [{ email: to }],
      subject: `Your bot "${botName}" is on the default model. 2 hours before service interruption`,
      htmlContent: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#111;">
          ...
          <p style="margin:0 0 16px;">You have a <strong>2-hour grace period</strong>, so your bots keep responding during this window. If you don't top up before it ends, your bots will be temporarily disabled until you add more credits.</p>
