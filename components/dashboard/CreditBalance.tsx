...
function PackCard({ packId }: { packId: PackId }) {
  ...
  const href = waHref(
    `Hi, I'd like to buy the ${label} credit pack (${tokensLabel} tokens) on Octively.`
  )
...
export function CreditBalance({ balance, transactions, appUrl: _appUrl, plan }: CreditBalanceProps) {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {(Object.keys(CREDIT_PACKS) as PackId[]).map((packId) => (
              <PackCard key={packId} packId={packId} />
            ))}
          </div>
          <p className="text-[11px] text-[var(--ink-muted)] mt-3" style={{ fontFamily: 'var(--font-mono)' }}>
            message us on WhatsApp to top up · credits never expire
          </p>
        </div>
      )}
