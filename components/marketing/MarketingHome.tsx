...
/* Hallmark · component: card-pair · genre: modern-minimal · theme: project (Octively)
 * differentiated cards (brand watermark vs. waveform), hover-lift, reduced-motion safe
 * states covered: default · hover · focus-visible (card link) · reduced-motion
 */

const WHATSAPP_GREEN = '#25D366'

// Official WhatsApp glyph (simple-icons path) — used as the floating brand watermark.
function WhatsAppGlyph({ size = 24, color = WHATSAPP_GREEN }: { size?: number; color?: string }) {
  return (
...
// Voice waveform motif — equalizer bars in the Sky-Teal accent.
function VoiceWaveform() {
  const bars = [22, 46, 70, 38, 88, 54, 30, 66, 42, 18]
  return (
    <svg width="190" height="96" viewBox="0 0 190 96" fill="none" aria-hidden="true" focusable="false">
      {bars.map((h, i) => (
        <rect key={i} x={i * 19 + 5} y={(96 - h) / 2} width="7" height={h} rx="3.5" fill="var(--of-primary)" />
      ))}
    </svg>
  )
}
...
const UPCOMING = [
  {
    ...
    watermark: (
      <span aria-hidden className="mkt-up-mark" style={{ position: 'absolute', right: -26, bottom: -36, lineHeight: 0, opacity: 0.07, transform: 'rotate(-8deg)', pointerEvents: 'none' }}>
        <WhatsAppGlyph size={210} />
      </span>
    ),
  },
  {
    key: 'voice',
    tile: <Mic size={19} style={{ color: 'var(--of-primary)' }} />,
    title: 'Voice input',
    ...
    watermark: (
      <span aria-hidden className="mkt-up-mark" style={{ position: 'absolute', right: 14, bottom: 16, opacity: 0.1, pointerEvents: 'none' }}>
        <VoiceWaveform />
      </span>
    ),
  },
    ...
    <section style={{ paddingBlock: 80, borderTop: '1px solid var(--hairline)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
        ...
        <Reveal>
          ...
          >
            {UPCOMING.map(({ key, tile, title, desc, watermark }) => (
              <div
                key={key}
                className="mkt-upcoming-card"
                style={{
                  position: 'relative',
                  overflow: 'hidden',
                  padding: 26,
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <h3 style={{ fontSize: 17.5, fontWeight: 650, letterSpacing: '-0.01em', marginBottom: 8, color: 'var(--ink)' }}>{title}</h3>
                  <p style={{ fontSize: 14, lineHeight: 1.65, color: 'var(--ink-muted)', margin: 0, maxWidth: '40ch' }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
...
function PricingTeaser() {
  ...
  const plans = [
    ...
    { key: 'starter', name: 'Starter', pkr: 2500, usd: 15, blurb: '2 bots · 3K convos · 30M credits/mo', feats: ['Unlimited leads · 25 MB storage · 30-day history', 'Full widget · lead capture · strict mode', 'WordPress plugin · email support'] },
    { key: 'pro', name: 'Pro', pkr: 7500, usd: 29, blurb: '8 bots · 15K convos · 150M credits/mo', feats: ['Unlimited leads · 100 MB storage · unlimited history', 'Enhanced responses · advanced analytics', 'Unanswered questions · smart routing', 'PDF upload · scraping · per-page analytics'], featured: true },
  ]
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', marginBottom: 32 }}>
        ...
        >
          From the developers and agencies Octively is built for.
        </p>
      </div>
