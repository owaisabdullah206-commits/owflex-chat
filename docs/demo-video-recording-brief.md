...
| `video/raw/admin-02-create-form.mp4` | Click **Create bot** → form opens. Fill all four fields slowly: **① Bot name** — "Noor Dental" · **② Store URL** — `https://noordentallahore.pk` · **③ Currency** — PKR · **④ System prompt** — click **"Insert example ↗"** so the professional template fills in automatically, pause 1.5s on the filled textarea, then scroll down to the button. | **Noor Dental** | Scroll slowly so every field is visible. The "Insert example" fill is a strong moment — let it breathe 1.5s before scrolling down. |
